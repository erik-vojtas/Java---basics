...
Static Binding (aka Early Binding): Binding resolved at compile time
Dynamic Binding (aka Late Binding): Binding resolved at run time
...
Adds a data type to a class. A class that implements a tagging interface becomes an interface type through polymorphism
▲ TOP
...
All of the fields must be serializable. If a field is not serializable, it must be marked with the transient keyword
...
Instantiate a Thread object, like so: Thread (Runnable threadObj, String threadName), where threadObj is an instance of a class that implements Runnable, and threadName is the name given to the new thread
Start a Thread object with start(), which executes a class to the class's run() method
...
After creating the Thread object, start it with the start() method
Thread Synchronization
...
public void wait() -- causes the current thread to wait until another thread invokes notify()
public void notify() -- wakes up a single thread that is waiting on the object's monitor
public void notifyAll() -- wakes up all threads that called wait() on the same object
...
Use ConcurrentHashMap instead of Hashtable when synchronization is needed.
...
Synchronization: Vector is synchronized, which means only one thread can access it at a time, while ArrayList is not synchronized, which means multiple threads could read it at the same time.
Performance: ArrayList is faster, as Vector incurs slight overhead in acquiring the lock.
Growth: Vector and ArrayList both grow and shrink dynamically, but ArrayList increments 50% of the current array size if the number of elements exceeds its capacity, while Vector increments 100%.
Traversal: Vector can use both Enumeration and Iterator for traversing elements, while ArrayList can only use Iterator.
